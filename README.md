...
```sh
git clone https://github.com/irgydaffa/Tes-Ordo.git
cd Tes-Ordo
```
...
### 1. Add order product lists on order detail (route: http://localhost:8000/user/order/show/{order_id})

-   Memperbarui tampilan harga pengiriman menggunakan helper `optional()` Laravel:
...
Menggunakan optional() helper - melindungi dari error jika relasi shipping tidak ada
Menggunakan null coalescing operator ?? - memberikan nilai default '0.00' jika price null
Mencegah error "Trying to get property of non-object" yang bisa terjadi jika shipping tidak ditemukan
